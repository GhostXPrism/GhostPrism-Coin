...
- [Development guidelines](#development-guidelines)
    - [General GHOST PRISM Coin](#general-ghost_prism-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
GHOST PRISM Coin uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Run with the `-testnet` option to run with "play GHOSTs (tGHOST)" on the test network, if you
...
GHOST PRISM Coin is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/ghost_prism-project/ghost_prism/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_ghost_prism
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_ghost_prism --log_level=test_suite
$ valgrind -v --leak-check=full src/ghost_prismd -printtoconsole
```
...
# A coverage report will now be accessible at `./test_ghost_prism.coverage/index.html`.
```
...
- BitcoinMiner : Generates GHOSTs (if wallet is enabled).
...
However, in open source software such as GHOST PRISM Coin, where everyone uses
...
pay attention to for reviewers of GHOST PRISM Coin code.

General GHOST PRISM Coin
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: GHOST PRISM Coin uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against GHOST PRISM Coin so that they can be integrated
...
file descriptors it will cause problems with GHOST_PRISM's `select()` loop, because
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:GHOST_PRISM-Project/GHOST_PRISM.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `ghost_prism-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `ghost_prism-cli` and the GUI debug console use this table to determine how to
