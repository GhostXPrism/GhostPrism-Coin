...
*Setup instructions for a gitian build of GHOST PRISM Coin using a VM or physical system.*

Gitian is the deterministic build process that is used to build the GHOST_PRISM
...
to the GHOST_PRISM GitHub release page.
...
- [Building GHOST PRISM Coin](#building-ghost_prism-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/ghost_prism-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/ghost_prism-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/ghost_prism-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/ghost_prism-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/ghost_prism-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/ghost_prism-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/ghost_prism-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@ghost_prism.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/ghost_prism-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/ghost_prism-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/ghost_prism-osx-*-build.assert
```
