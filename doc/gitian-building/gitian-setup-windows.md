...
The build script we'll be using is contained in the GHOST_PRISM github repository ([contrib/gitian-build.py](https://github.com/ghost_prism-project/ghost_prism/blob/master/contrib/gitian-build.py)). Since this is a completely fresh environment, we haven't yet cloned the GHOST_PRISM repository and will need to fetch this script with the following commands:

```bash
curl -L -O https://raw.githubusercontent.com/GHOST_PRISM-Project/GHOST_PRISM/master/contrib/gitian-build.py
...
- Clone the gitian-builder, gitian.sigs, ghost_prism-detached-sigs, and ghost_prism GitHub repos.
