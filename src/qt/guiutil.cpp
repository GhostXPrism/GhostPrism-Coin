...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a GHOST_PRISM address (e.g. %1)").arg("GSfaHsKBwdi7d4xeXCgQ3tzNkWzvxchSjN"));
...
{
    // return if URI is not valid or is no GHOST_PRISM: URI
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        if (i->first == "message") {
            ...
            if (!i->second.isEmpty()) {
                if (!BitcoinUnits::parse(BitcoinUnits::GHOST, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
{
    // Convert ghost_prism:// to ghost_prism:
    //
    //    Cannot handle this later, because ghost_prism:// will cause Qt to see the part after // as host,
...
QString formatBitcoinURI(const SendCoinsRecipient& info)
{
    ...
    if (info.amount) {
        ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::GHOST, info.amount, false, BitcoinUnits::separatorNever));
        paramCount++;
    }
...
void openConfigfile()
{
    ...
    /* Open ghost_prism.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "GHOST_PRISM.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for GHOST_PRISM.lnk
...
{
    return GetAutostartDir() / "ghost_prism.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a ghost_prism.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=GHOST_PRISM\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the ghost_prism app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add ghost_prism app to startup item list
