...
/**
 * Fees smaller than this (in ughost) are considered zero fee (for transaction creation)
		...
		for (std::map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it) {
			...
			for (unsigned int i = 0; i < pcoin->vout.size(); i++) {
				...
				bool fIsValid = (
					...
					((mine & (ISMINE_MULTISIG | ISMINE_NO)) != ISMINE_NO));
...
std::map<CBitcoinAddress, std::vector<COutput> > CWallet::AvailableCoinsByAddress(bool fConfirmed, CAmount maxCoinValue)
{
	...
	for (COutput out : vCoins) {
		...
		mapCoins[CBitcoinAddress(address, CChainParams::PUBKEY_ADDRESS)].push_back(out);
	}
...
{
	return AvailableCoins(pCoins, true, nullptr, false, STAKEABLE_COINS);
}
...
bool CWallet::GetBudgetSystemCollateralTX(CWalletTx& tx, uint256 hash, bool useIX)
{
	...
	vecSend.push_back(std::make_pair(scriptChange, BUDGET_FEE_TX_OLD)); // Old 50 GHOST collateral
...
bool CWallet::GetBudgetFinalizationCollateralTX(CWalletTx& tx, uint256 hash, bool useIX)
{
	...
	std::string strFail = "";
	std::vector<std::pair<CScript, CAmount> > vecSend;
	vecSend.push_back(std::make_pair(scriptChange, BUDGET_FEE_TX)); // New 5 GHOST collateral
			...
			while (true) {
				...
				if (!SelectCoins(nTotalValue, setCoins, nValueIn, coinControl, coin_type, useIX)) {
					if (coin_type == ALL_COINS) {
						...
						strFailReason = _("Unable to locate enough funds for this transaction that are not equal 10000 GHOST.");
					}
					else if (coin_type == ONLY_NONDENOMINATED_NOT10000IFMN) {
						strFailReason = _("Unable to locate enough Obfuscation non-denominated funds for this transaction that are not equal 10000 GHOST.");
					}
					else {
				...
				if (nChange > 0) {
					...
					// change transaction isn't always pay-to-ghost_prism-address
					CScript scriptChange;
...
)
{
	...
	// Parse utxos into CGhostStakes
	std::list<std::unique_ptr<CStakeInput> > listInputs;
	for (const COutput &out : vCoins) {
		std::unique_ptr<CGhostStake> input(new CGhostStake());
