...
bool CGhostStake::SetInput(CTransaction txPrev, unsigned int n)
{
    this->txFrom = txPrev;
    this->nPosition = n;
    return true;
...
bool CGhostStake::GetTxFrom(CTransaction& tx) const
{
    tx = txFrom;
    return true;
}

bool CGhostStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(txFrom.GetHash(), nPosition);
    return true;
}

CAmount CGhostStake::GetValue() const
{
    return txFrom.vout[nPosition].nValue;
}

bool CGhostStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal)
{
...
CDataStream CGhostStake::GetUniqueness() const
{
    //The unique identifier for a GHOST stake is the outpoint
...
CBlockIndex* CGhostStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
    uint256 hashBlock = 0;
