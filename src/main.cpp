...
#if defined(NDEBUG)
#error "GHOST_PRISM cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in ughost) are considered zero fee (for relaying and mining)
...
int64_t GetBlockValue(int nHeight)
{
	...
	else if (nHeight <= 10000 && nHeight > last_pow_block) {
		nSubsidy = 1.50 * COIN;
	}
	...
	else if (nHeight <= 70000 && nHeight >= 40001) {
		nSubsidy = 3 * COIN;
	}
	...
	else if (nHeight <= 100000 && nHeight >= 90001) {
		nSubsidy = 3.75 * COIN;
	}
	else if (nHeight <= 110000 && nHeight >= 100001) {
		nSubsidy = 3.90 * COIN;
	}
	else if (nHeight <= 120000 && nHeight >= 110001) {
		nSubsidy = 4 * COIN;
	}
	else if (nHeight <= 130000 && nHeight >= 120001) {
		nSubsidy = 4.5 * COIN;
	}
	else if (nHeight <= 190000 && nHeight >= 130001) {
		nSubsidy = 5 * COIN;
	}
	else if (nHeight <= 230000 && nHeight >= 190001) {
		nSubsidy = 4.50 * COIN;
	}
	else if (nHeight <= 270000 && nHeight >= 230001) {
		nSubsidy = 4.25 * COIN;
	}
	else if (nHeight <= 290000 && nHeight >= 270001) {
		nSubsidy = 4.50 * COIN;
	}
	else if (nHeight <= 300000 && nHeight >= 290001) {
		nSubsidy = 4.55 * COIN;
	}
	else if (nHeight <= 37703108 && nHeight >= 300001) {
		nSubsidy = 4.60 * COIN;
	}
	else if (nHeight == 37703109) {
		nSubsidy = 3.20 * COIN;
	}
	else {
		nSubsidy = 0 * COIN;
...
{
	util::ThreadRename("ghost_prism-scriptch");
...
bool CheckWork(const CBlock block, CBlockIndex* const pindexPrev)
{
	...
	if (block.nBits != nBitsRequired) {
		// Ghost_Prism Specific reference to the block with the wrong threshold was used.
		const Consensus::Params& consensus = Params().GetConsensus();
		if ((block.nTime == (uint32_t)consensus.nGhost_PrismBadBlockTime) &&
			(block.nBits == (uint32_t)consensus.nGhost_PrismBadBlockBits)) {
			// accept GHOST_PRISM block minted with incorrect proof of work threshold
			return true;
		}
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
	...
	if (isPoS) {
		...
		// Inputs
		std::vector<CTxIn> ghostInputs;

		for (const CTxIn& stakeIn : stakeTxIn.vin) {
			ghostInputs.push_back(stakeIn);
		}
		const bool hasGHOSTInputs = !ghostInputs.empty();
		...
		for (const CTransaction& tx : block.vtx) {
			for (const CTxIn& in : tx.vin) {
				...
				if (hasGHOSTInputs) {
					...
					for (const CTxIn& ghostIn : ghostInputs)
						if (ghostIn.prevout == in.prevout)
			...
			while (!chainActive.Contains(prev)) {
				...
				for (const CTransaction& t : bl.vtx) {
					...
					for (const CTxIn& in : t.vin) {
						...
						if (hasGHOSTInputs) {
							for (const CTxIn& stakeIn : ghostInputs)
...
bool static ProcessMessage(CNode* pfrom, std::string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
	...
	if (strCommand == "version") {
		...
		// GHOST_PRISM: We use certain sporks during IBD, so check to see if they are
