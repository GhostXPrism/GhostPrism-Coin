...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != g_best_block)
            return error("GHOST_PRISMMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock)) {
        return error("GHOST_PRISMMiner : ProcessNewBlock, block not accepted");
    }
...
{
    LogPrintf("GHOST_PRISMMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    util::ThreadRename("ghost_prism-miner");
  ...
  //     }
        ...
        LogPrintf("Running GHOST_PRISMMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("GHOST_PRISMMiner exception");
    } catch (...) {
        LogPrintf("GHOST_PRISMMiner exception");
    }

    LogPrintf("GHOST_PRISMMiner exiting\n");
}
