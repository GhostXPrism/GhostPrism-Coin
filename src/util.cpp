...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL) {
        ...
        if (ptrCategory.get() == NULL) {
            ...
            // "ghost_prism" is a composite category enabling all GHOST_PRISM-related debug output
            if (ptrCategory->count(std::string("ghost_prism"))) {
...
#else
    const char* pszModule = "ghost_prism";
#endif
    if (pex)
        return strprintf(
...
// Windows < Vista: C:\Documents and Settings\Username\Application Data\GHOST_PRISM
// Windows >= Vista: C:\Users\Username\AppData\Roaming\GHOST_PRISM
// Mac: ~/Library/Application Support/GHOST_PRISM
// Unix: ~/.ghost_prism
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "GHOSTPRISM";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "ghost_prism.conf"));
    ...
    std::map<std::string, std::vector<std::string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty ghost_prism.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override ghost_prism.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "ghost_prismd.pid"));
